...
template<>
struct FilterAPI<dtl::cf> {

  using filter_t = dtl::cf;
  using word_t = typename filter_t::word_t;

  static constexpr u1 supports_concurrent_inserts = false;

  static filter_t construct(const std::size_t bitlength, const uint32_t bits_per_tag, const uint32_t tags_per_bucket) {
    filter_t bf(bitlength, bits_per_tag, tags_per_bucket);
    return std::move(bf);
  }
  ...
  add(uint64_t key, filter_t* filter, word_t* __restrict filter_data) {
    if (!filter->insert(filter_data, key)) {
