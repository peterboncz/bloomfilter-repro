...
class BetterTableCounting {
 public:
  ...
  static constexpr bool counting_supported = true; // a counting cuckoo table stores duplicates
...
//  Bucket *buckets_;
 ...
 public:
  explicit BetterTableCounting(const size_t num) : num_buckets_(num) {
//    std::cout << "num buckets: " << num << std::endl;
  ...
  std::string Info() const {
    std::stringstream ss;
    ss << "Cuckoo table (BetterTableCounting):\n";
...
//      size_t r = dtl::this_thread::rand32() % kTagsPerBucket;
