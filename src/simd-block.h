...
template<typename HashFamily = ::cuckoofilter::TwoIndependentMultiplyShift, typename KeyType = uint64_t>
...
private:
  ...
  static constexpr uint32_t entry_cnt_per_bucket = 8;
  using Bucket = word_t[entry_cnt_per_bucket];
...
//      directory_(that.directory_),
  ...
  void Add(word_t* __restrict filter_data, const KeyType key) noexcept;
  bool Find(const word_t* __restrict filter_data, const KeyType key) const noexcept;
  uint64_t SizeInBytes() const { return sizeof(Bucket) * (1ull << log_num_buckets_); }
  std::size_t size() const { return SizeInBytes() / sizeof(word_t); }
...
template<typename HashFamily, typename KeyType>
SimdBlockFilter<HashFamily, KeyType>::SimdBlockFilter(const int log_heap_space)
...
//    directory_(nullptr),
  ...
  alloc_size = 1ull << (log_num_buckets_ + LOG_BUCKET_BYTE_SIZE);
//  const int malloc_failed =
//      posix_memalign(reinterpret_cast<void**>(&directory_), 64, alloc_size);
//  if (malloc_failed) throw ::std::bad_alloc();

//  dtl::mem::numa_allocator<uint8_t> allocator;
//  directory_ = reinterpret_cast<Bucket*>(allocator.allocate(alloc_size));
//  if (directory_ == nullptr) throw ::std::bad_alloc();
//  memset(directory_, 0, alloc_size);
}

template<typename HashFamily, typename KeyType>
SimdBlockFilter<HashFamily, KeyType>::~SimdBlockFilter() noexcept {
//  dtl::mem::numa_allocator<uint8_t> allocator;
//  allocator.deallocate(reinterpret_cast<uint8_t*>(directory_), alloc_size);
//  free(directory_);
//  directory_ = nullptr;
}
...
SimdBlockFilter<HashFamily, KeyType>::MakeMask(const uint32_t hash) noexcept {
...
SimdBlockFilter<HashFamily, KeyType>::Add(word_t* __restrict filter_data, const KeyType key) noexcept {
  ...
  __m256i* const bucket = &reinterpret_cast<__m256i*>(filter_data)[bucket_idx];
...
bool
SimdBlockFilter<HashFamily, KeyType>::Find(const word_t* __restrict filter_data, const KeyType key) const noexcept {
  const auto hash = hasher_(key);
  const uint32_t bucket_idx = hash & directory_mask_;
  const __m256i mask = MakeMask(hash >> log_num_buckets_);
  const __m256i bucket = reinterpret_cast<const __m256i*>(filter_data)[bucket_idx];
